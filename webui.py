...
def generar_imagenes_desde_excel(excel_file, currentTask):
    """Función para generar imágenes a partir de prompts de un archivo Excel."""

    try:
        prompts_df = pd.read_excel(excel_file.name)
        prompts_list = prompts_df.iloc[:, 0].tolist()

        # Crea una lista de tareas (una por cada prompt)
        tasks = []
        for i, prompt in enumerate(prompts_list):
            # Obtiene la semilla actual (o genera una nueva si es aleatoria)
            seed = modules.config.refresh_seed(modules.config.seed_random, modules.config.image_seed) if i == 0 else (int(modules.config.image_seed) + i) % (2**32 - 1)
            ...
            task_args = [
                ...
                1,  # image_number (genera una imagen por prompt)
                ...
                modules.config.default_base_model_name,  # base_model
                modules.config.default_refiner_model_name,  # refiner_model
                modules.config.default_refiner_switch,  # refiner_switch
            ]

            # Agrega los parámetros para cada LoRA
            for lora in modules.config.default_loras:
                task_args.extend(lora)

            task_args.extend([
                False,  # input_image_checkbox
                'uov',  # current_tab
                modules.config.disabled,  # uov_method
                ...
                modules.config.refiner_swap_method,  # refiner_swap_method
                ...
                modules.config.MetadataScheme.FOOOCUS.value,  # metadata_scheme
            ])
            
            # Agrega los parámetros para cada "Image Prompt" (si es necesario)
            ...
            tasks.append(worker.AsyncTask(args=task_args))

        # Ejecuta las tareas una a una
        for task in tasks:
            yield from generate_clicked(task)
...
with shared.gradio_root:
    ...
    #añadido
    with gr.Row(elem_classes='additional_modules'):
        with gr.Column(scale=1):
            gr.Markdown("### Otra forma de generar imágenes")
            with gr.Row():
                excel_file = gr.File(label="Archivo Excel con Prompts", type="file")
                generate_excel_button = gr.Button(label="Generar desde Excel")


    generate_excel_button.click(
        generar_imagenes_desde_excel,
