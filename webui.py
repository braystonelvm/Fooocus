...
def get_prompt_from_excel(file_obj):
  """Lee el prompt de un archivo Excel.

  Args:
    file_obj: Objeto de archivo Gradio.

  Returns:
    El prompt leído del archivo Excel.
  """
  try:
...
with shared.gradio_root:
    ...
    # Nueva sección para cargar el prompt desde Excel
    with gr.Row():
        gr.Markdown("## Otra forma de generar imagenes")

    with gr.Row():
        with gr.Column(scale=2):
            excel_file = gr.File(label="Archivo Excel con el Prompt", file_types=[".xlsx", ".xls"])
            generate_excel_button = gr.Button(label="Generar desde Excel", value="Generar desde Excel", elem_classes='type_row', elem_id='generate_excel_button')
...
# Conectar el botón de generación con la función de lectura del prompt
generate_excel_button.click(
    fn=get_prompt_from_excel,
    inputs=excel_file,
    outputs=prompt,
).then(fn=refresh_seed, inputs=[seed_random, image_seed], outputs=image_seed) \
.then(fn=get_task, inputs=ctrls, outputs=currentTask) \
